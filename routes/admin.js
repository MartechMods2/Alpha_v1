...
import { createPairingInvite, consumePairingInvite, pairingInviteStats } from "../utils/pairingInvites.js";
...
// Minimal probes for hosts and uptime monitors. They never expose credentials.
router.get("/healthz", (_req, res) => res.json({ status: "ok", uptime: Math.floor(process.uptime()) }));
router.get("/readyz", async (req, res) => {
	try {
		await mdClient.db("admin").command({ ping: 1 });
		const connected = Boolean(req.app.locals.sock?.user);
		return res.status(connected ? 200 : 503).json({ status: connected ? "ready" : "degraded", database: "ready", whatsapp: connected ? "connected" : "disconnected" });
	} catch (_) {
		return res.status(503).json({ status: "not-ready", database: "unavailable", whatsapp: "unknown" });
	}
});

router.post("/api/admin/ws-ticket", requireAdmin, (_req, res) => res.json({ ok: true, ...createAdminWebsocketTicket() }));

// ── One-time public pairing endpoint ─────────────────────────────────────────
// A customer needs an expiring invitation generated by an authenticated admin.
router.post("/api/pair", async (req, res) => {
	const { phoneNumber, invite } = req.body;
	...
	try {
		const normalized = normalizeLookupNumber(phoneNumber);
		if (!normalized) return res.status(400).json({ error: "Invalid phone number. Nigerian local numbers and international +country-code formats are supported." });
		if (!consumePairingInvite(invite)) return res.status(403).json({ error: "This pairing invitation is missing, expired or already used. Ask Martech for a new link." });
		const code = await sock.requestPairingCode(normalized.number);
...
router.post("/api/admin/pair-invite", requireAdmin, (req, res) => {
	const sock = req.app.locals.sock;
	if (sock?.authState?.creds?.registered) return res.status(409).json({ error: "This deployment is already paired. Create a separate deployment for each customer." });
	const created = createPairingInvite();
	const origin = String(process.env.HOST_URL || `${req.protocol}://${req.get("host")}`).replace(/\/$/, "");
	res.json({ ok: true, url: `${origin}/?pair_invite=${encodeURIComponent(created.token)}`, expiresAt: created.expiresAt, ...pairingInviteStats() });
});
	...
	try {
		...
		res.json({
			...
			integrations: {
				...
				safeSendApi: integrationApiConfigured(),
