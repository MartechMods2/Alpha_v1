...
const askNvidia = async (systemPrompt, messages) => {
	...
	const response = await fetch("https://integrate.api.nvidia.com/v1/chat/completions", {
		...
		body: JSON.stringify({ model: process.env.NVIDIA_AI_MODEL || "openai/gpt-oss-20b", messages: [{ role: "system", content: systemPrompt }, ...messages], temperature: 0.5, max_tokens: 700 }),
	});
	if (!response.ok) throw new Error(`NVIDIA ${response.status}`);
	const data = await response.json();
	const text = data?.choices?.[0]?.message?.content?.trim();
	if (!text) throw new Error("NVIDIA returned no text");
	return text;
};
...
const askGemini = async (systemPrompt, messages) => {
	...
	const model = new GoogleGenerativeAI(process.env.GOOGLE_API_KEY).getGenerativeModel({
		model: process.env.GEMINI_TEXT_MODEL || process.env.GEMINI_MEDIA_MODEL || "gemini-2.0-flash",
...
export const askSafeAi = async ({ groupJid = "direct", systemPrompt, messages }) => {
	...
	const safeMessages = messages.map((x) => ({ ...x, content: settings.aiPiiRedaction === false ? String(x.content) : redactPii(x.content) }));
	...
	for (const [name, fn] of [["nvidia", askNvidia], ["gemini", askGemini]]) {
		try {
			const text = await fn(effectiveSystemPrompt, safeMessages);
			note(name, true);
			return { text, provider: name };
		} catch (error) {
			note(name, false, error.message);
