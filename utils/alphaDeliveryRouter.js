...
export const handleExplicitAlphaDelivery = async ({ sock, msg, groupJid, senderJid, body }) => {
	...
	// Critical rule: if the user does not specify a delivery format, Alpha stays text-first.
	...
	if (!mentionedList.some((jid) => isSameGroupUser(metadata, jid, botJids))) return false;
